...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [{ url: publicUrl('/'), lastModified: latest }, { url: publicUrl('/methodology') }, { url: publicUrl('/methodology/shopping-journey-v2') }, { url: publicUrl('/request') }, { url: publicUrl('/privacy') }, { url: publicUrl('/methodology/quality-pilot-v1') }, { url: publicUrl('/studies') }, { url: publicUrl('/studies/policy-resolution-v1') }, ...research.studies.map(s => ({ url: publicUrl(`/studies/${s.slug}`), lastModified: new Date(s.publishedAt) })), ...profiles.values(), ...listReports().map(r => ({ url: publicUrl(`/reports/${r.slug}`), lastModified: new Date(r.publishedAt) }))];
}
