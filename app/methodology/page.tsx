...
export default function Page() {
  return <main id="main" className="shell method-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd(pageStructuredData({ type: 'TechArticle', path: '/methodology', headline: 'Research methodology and scoring', description: 'How Alhena Research Lab measures policy-compliant resolution, answer quality and speed, and combines them into shopping and support composite scores.', crumbs: [{ name: 'Methodology', path: '/methodology' }] })) }} />
    ...
    <section className="library-method-note"><h2>New: shopping across the storefront</h2><p><strong>shopping-journey-v2</strong> adds six shopping outcome tasks and a separate measure of usable shopping reach across chat, product-page Q&A, recommendations, search and cart assistance. More distinct, verified usable interfaces earn reach credit; duplicate widgets do not. Outcomes and reach remain separate scores.</p><p>The framework is published, with no results under this version yet. Automated capture across these interfaces is not enabled. Current submissions and published scores retain their existing method; support scoring is unchanged.</p><Link className="button primary" href="/methodology/shopping-journey-v2">Explore the shopping journey framework</Link></section>
    <p className="intro">The current research library uses <strong>policy-resolution-v1</strong>. Shopping and support composite scores combine three distinct measures: policy-compliant resolution, answer quality and full-answer speed. Studies test one form factor, the AI agent in a storefront’s on-site chat widget; helpdesks, email, SMS, voice, search, product-page content and AI visibility are outside their scope.</p>
