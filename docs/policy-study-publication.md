...
The catalog contains only validated releases: reviewed offline studies installed with publication approval, and approved submitted evaluations published automatically after server validation. This document describes the offline installer; see [automated policy evaluations](automated-policy-evaluations.md) for the submitted-tool publication path. The website reads these immutable releases; changing the presentation does not recalculate scores or upgrade the production evaluation worker.
...
Original `/reports` and quality-only profiles remain in the historical archive. `/quality-pilot-scores.json` preserves the original v1 machine summary and `/methodology/quality-pilot-v1` preserves the pilot method. `/methodology` is a hub that leads with the current policy-resolution method and explains both scopes. New submissions use policy-resolution-v1: three supplied storefronts plus two researched deployments, followed by approval of all five and automatic publication after validation. Existing quality-pilot jobs retain their original scope. Compatible reuse remains limited to 30 days. Private evidence remains protected under both methods.
