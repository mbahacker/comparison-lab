...
Reviewed handoff queue UI is treated as a partial conversation boundary, not as an AI answer. Positively attributed AI text remains available for assessment; incomplete response timing stays null, and later questions remain unsubmitted. A retained attribution failure can be reconciled only when its exact observed suffix and complete author proof establish that boundary. Operator recovery binds the original capture, approved scenario and send journal by hash, preserves the original failure, and never resends its questions.

A CLI exit code alone does not identify a retryable model failure. For exit code 1, the judge recognizes only a same-session structured error result declaring `error_max_turns` or `error_max_structured_output_retries`, with no verdict or permission denial. These map to the existing bounded incomplete-output/schema retry path. Unknown process, authentication and execution failures remain `cli_failed` and require operator review. Raw CLI diagnostics are never returned or logged. No model, prompt, turn limit or score changes are made by this recovery.
