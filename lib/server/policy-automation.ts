...
export function mergeComparisonRepairs(current:Row|null,original:Row|null,provider:string) {
  const sources=[current,original&&{...original,candidates:original.candidates.filter((c:Row)=>c.provider===provider)}].filter(s=>s?.candidates?.length) as Row[];
  if(!sources.length)return null;
  const candidates=sources.flatMap(s=>s.candidates);
  if(new Set(candidates.map((c:Row)=>c.id)).size!==candidates.length)throw Error('Duplicate comparison repair provenance');
  return {selectionRule:[...new Set(sources.map(s=>s.selectionRule))].join('\n'),sourceClassificationNote:[...new Set(sources.map(s=>s.sourceClassificationNote).filter(Boolean))].join('\n'),candidates};
}
