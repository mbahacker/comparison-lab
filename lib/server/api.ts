...
import { ApiError, type ComparisonRequest, type Row } from './model.ts';
import { hash, iso, multilineText, otp, otpDigest, providersInput, rateLimit, requireOrigin, requireUser, requireWorker, safeEqual, sessionCookie, sessionUser, text, token, workEmail, SESSION_COOKIE } from './security.ts';
...
const json = (body: unknown, status = 200, headers: HeadersInit = {}) => new Response(JSON.stringify(body), { status, headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store', 'x-content-type-options': 'nosniff', ...headers } });
...
async function startAuth(request: Request) {
  ...
  const email = workEmail(input.email); const name = text(input.name, 'Name', 2, 120);
  ...
  const mailId = transaction(() => {
    ...
    return enqueueMail(`verify:${randomUUID()}`, email, 'Your Comparison Lab verification code', `Hi ${name},\n\nYour verification code is: ${code}\n\nIt expires in 10 minutes. Enter it in the browser where you requested it. If you did not request this code, you can ignore this message.\n\nComparison Lab verifies mailbox ownership. It does not certify employment or endorse a company.`);
  });
...
async function verifyAuth(request: Request) {
  ...
  const result = transaction(() => {
    ...
    const now = iso();
    db().prepare('INSERT INTO users (id,email,name,verified_at,created_at) VALUES (?,?,?,?,?) ON CONFLICT(email) DO UPDATE SET name=excluded.name,verified_at=excluded.verified_at').run(randomUUID(), email, challenge.name, now, now);
  ...
  return json({ user: result.user }, 200, { 'set-cookie': sessionCookie(rawSession) });
}

async function submitRequest(request: Request) {
  const user = requireUser(request); const input = await body(request);
...
async function reviewRequest(request: Request, rawToken: string) {
  if (request.method === 'GET') {
    ...
    return json({ request: requestView(row), requester: { name: user.name, email: user.email }, expiresAt: new Date(row.review_expires_at).toISOString(), decided: !!row.review_decision });
  }
  const input = await body(request);
  ...
  const id = transaction(() => {
    ...
    if (input.decision === 'approve') {
      db().prepare('INSERT INTO jobs (id,request_id,state,protocol_json,available_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?)').run(randomUUID(), row.id, 'queued', JSON.stringify(approvedProtocol), Date.now(), now, now);
      enqueueMail(`request:${row.id}:approved`, user.email, `Comparison approved: ${requestTitle(row)}`, `Hi ${user.name},\n\nYour comparison has been approved and is queued for a fixed-rubric evaluation. Unsupported storefronts or incomplete evidence will pause the run for review.\n\nPrivate status:\n${statusUrl(row.id)}\n\nWe will send the public report when capture, judging, audit and validation are complete.`);
    } else {
...
async function claimJob() {
  const job = transaction(() => {
    ...
    return { id: selected.id, requestId: selected.request_id, leaseToken, fencingToken, leaseExpiresAt: new Date(expires).toISOString(), attempt: selected.attempt + 1, providers: JSON.parse(request.providers_json), protocol: JSON.parse(selected.protocol_json), approvedAt: request.reviewed_at, attributionConfirmedAt: request.attribution_confirmed_at };
  });
  return json({ job });
}
...
async function completeJob(request: Request) {
  ...
  const counts = validateEvidence(input.evidence, JSON.parse(row.providers_json), JSON.parse(job.protocol_json), [user.email, user.name, config().adminEmail, input.leaseToken, config().workerSecret]);
...
export async function handleApi(request: Request, segments?: string[]): Promise<Response> {
  try {
    ...
    if (parts[0] === 'reports' && parts.length >= 2 && method === 'GET') {
      ...
      if (parts.length === 2) return json(result);
      if (parts[2] === 'evidence' && parts.length === 3) return json(result.evidence, 200, { 'content-disposition': `attachment; filename="${parts[1]}-evidence.json"` });
      if (parts[2] === 'html' && parts.length === 3 && parts[1] === SEED_SLUG) {
        return new Response(fs.readFileSync(path.join(config().contentDir, SEED_SLUG, 'report.html'), 'utf8'), { headers: { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'public, max-age=3600', 'x-content-type-options': 'nosniff', 'content-security-policy': "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data:; font-src data:; connect-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'self'" } });
      }
    }
    throw new ApiError(404, 'Endpoint not found.');
  } catch (error) {
    if (error instanceof ApiError) return json({ error: error.message }, error.status);
    console.error('[Comparison Lab API]', error instanceof Error ? error.message : 'Unknown error');
