...
async function reviewRequest(request: Request, rawToken: string) {
  ...
  const id = transaction(() => {
    ...
    if (input.decision === 'approve') {
      ...
      enqueueMail(`request:${row.id}:approved${existingJob ? ':' + now : ''}`, user.email, `${JSON.parse(row.providers_json).length === 1 ? 'Tool evaluation' : 'Comparison'} approved: ${requestTitle(row)}`, `Hi ${user.name},\n\nYour evaluation has been approved. ${existingJob ? 'The corrected storefront list resumes the existing run. Compatible completed captures and scoring receipts for unchanged storefronts are retained with their original dates; only missing work is performed.' : `The current plan reuses ${reuse.reusedConversations} previously evaluated conversations and runs ${reuse.newConversations} new conversations.`} Only compatible analysis captured within 30 days is reused; its original dates and limitations remain visible. Unsupported storefronts or incomplete evidence will pause the run for review.\n\nPrivate status:\n${statusUrl(row.id)}\n\nWe will send the report link when capture, judging, audit and validation are complete.\n\nAlhena Research Lab`);
    } else {
