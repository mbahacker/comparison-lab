...
The homepage has Latest results, Comparative studies and Historical quality pilot archive views, with public tables, charts and original capture dates. New submissions use the five-storefront policy-resolution method. `/tools/[id]` profiles use stable canonical website identities and show the latest published policy-resolution study, with earlier quality-only results preserved separately. Compatible comparisons retain complete source cohorts rather than blending arbitrary stores across past runs. Single-tool evidence is stored as an immutable report. Derived pair reports retain hashes and provenance; their rows, the source evaluation, completion state and email events commit together. Existing pair reports and private request URLs remain valid.
